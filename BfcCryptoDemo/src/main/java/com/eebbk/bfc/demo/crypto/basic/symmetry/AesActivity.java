...
import com.eebbk.bfc.crypto.secretkey.symmetry.AESKeyManager;
...
import com.eebbk.bfc.demo.crypto.util.AESUtils;
import com.eebbk.bfc.demo.crypto.util.StringUtils;

import java.util.Arrays;

public class AesActivity extends BasicActivity implements View.OnClickListener{
    ...
    private String[] MODE=new String[]{AESCrypto.SymmetryMode.CBC,
            AESCrypto.SymmetryMode.ECB,
            AESCrypto.SymmetryMode.CTR,
            AESCrypto.SymmetryMode.CTS,
            AESCrypto.SymmetryMode.CFB,
            AESCrypto.SymmetryMode.OFB};
        //"CFB8","CFB16","CFB32","CFB64","OFB8","OFB16","OFB32","OFB64"

    private String[] PADDIND=new String[]{AESCrypto.SymmetryPadding.PKCS5_PADDING,
            AESCrypto.SymmetryPadding.NO_PADDING,
            AESCrypto.SymmetryPadding.ISO10126_PADDING};
        //,"ANSIX923PADDING","ZEROPADDING","PKCS7PADDING"

    private String[] KEY_LENGTH=new String[]{
            AESKeyManager.AESKeyLength.LENGTH_128 +"",
            AESKeyManager.AESKeyLength.LENGTH_192 +"",
            AESKeyManager.AESKeyLength.LENGTH_256 +""};
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if(isDefault){
                    ...
                    try{
                        resultString= AESUtils.encrypt(data);
                        byte[] bytesDate=data.getBytes("utf-8");
                        length=bytesDate.length;
                        resultByte=AESUtils.encrypt(bytesDate);
                    }catch (Exception e){
                        e.printStackTrace();
                    }
...
//            String key=mSecretEt.getText().toString().trim();
                    ...
                    AESCrypto crypto=new AESCrypto(getKey(),mCurrentMode,mCurrentPadding,iv.getBytes());
    ...
    private void handleDecrypt(){
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if(isDefault){
                    resultString=AESUtils.decrypt(encryptString);
                    resultByte=AESUtils.decrypt(StringUtils.getBytesFromString(encryptByte));
                }else if(isCustom){
//            String key=mSecretEt.getText().toString().trim();
                    String iv=mIvEt.getText().toString().trim();
                    AESCrypto crypto=new AESCrypto(getKey(),mCurrentMode,mCurrentPadding,iv.getBytes());
...
//        mKeyAutoBtn.setVisibility(View.VISIBLE);
        ...
        try {
            deSedeKeyManager = new DESedeKeyManager(mCurrentKeyLength);
        } catch (Exception e) {
            e.printStackTrace();
        }
