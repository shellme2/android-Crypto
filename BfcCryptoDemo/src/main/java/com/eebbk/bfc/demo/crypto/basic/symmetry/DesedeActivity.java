...
import com.eebbk.bfc.crypto.secretkey.symmetry.DESedeKeyManager;
...
import com.eebbk.bfc.demo.crypto.util.DESedeUtils;
import com.eebbk.bfc.demo.crypto.util.StringUtils;

import java.util.Arrays;

public class DesedeActivity extends BasicActivity implements View.OnClickListener{
    ...
    private String[] MODE=new String[]{DESedeCrypto.SymmetryMode.CBC,
            DESedeCrypto.SymmetryMode.ECB,
            DESedeCrypto.SymmetryMode.CTR,
            DESedeCrypto.SymmetryMode.CTS,
            DESedeCrypto.SymmetryMode.CFB,
            DESedeCrypto.SymmetryMode.OFB};
        // "PCBC","CFB8","CFB16","CFB32","CFB64",, "OFB8","OFB16","OFB32","OFB64"

    private String[] PADDIND=new String[]{DESedeCrypto.SymmetryPadding.PKCS5_PADDING,
            DESedeCrypto.SymmetryPadding.NO_PADDING,
            DESedeCrypto.SymmetryPadding.ISO10126_PADDING};//,"PKCS7PADDING"

    private String[] KEY_LENGTH=new String[]{
            DESedeKeyManager.DESedeKeyLength.LENGTH_128 +"",
            DESedeKeyManager.DESedeKeyLength.LENGTH_192 +""};
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if(isDefault){
                    ...
                    try{
                        resultString= DESedeUtils.encrypt(data);
                        byte[] bytesDate=data.getBytes("utf-8");
                        length=bytesDate.length;
                        resultByte=DESedeUtils.encrypt(bytesDate);
                    }catch (Exception e){
                        e.printStackTrace();
                    }
...
//            String key=mSecretEt.getText().toString().trim();
                    ...
                    DESedeCrypto crypto=new DESedeCrypto(getKey(),mCurrentMode,mCurrentPadding,iv.getBytes());
    ...
    private void handleDecrypt(){
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if(isDefault){
                    resultString=DESedeUtils.decrypt(encryptString);
                    resultByte=DESedeUtils.decrypt(StringUtils.getBytesFromString(encryptByte));
                }else if(isCustom){
//            String key=mSecretEt.getText().toString().trim();
                    String iv=mIvEt.getText().toString().trim();
                    DESedeCrypto crypto=new DESedeCrypto(getKey(),mCurrentMode,mCurrentPadding,iv.getBytes());
...
//        mKeyAutoBtn.setVisibility(View.VISIBLE);
        ...
        DESedeKeyManager deSedeKeyManager= null;
        try {
            deSedeKeyManager = new DESedeKeyManager(mCurrentKeyLenth);
        } catch (Exception e) {
            e.printStackTrace();
        }
//        Log.i("liuyewu","length=="+mCurrentKeyLenth);
        String key=Arrays.toString(deSedeKeyManager.getSecretKey().getEncoded());//new String(deSedeKeyManager.getSecretKey().getEncoded());//
